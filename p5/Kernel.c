code Kernel
  ...
  -- Harsukh Singh 
  -- Project 5 11/30/2015
  -- Instructor: Mark Morissey
  -- Notes on code:
  -- code attribution: the code for parts 1, 2 and 4 are completely written by me without any outside help and only using the instructions
  -- part 3 (frame manager) however I used the formula found in the book to find the frame address and also collaborated with two others but code was not shared
-----------------------------  InitializeScheduler  ---------------------------------
  ...
  behavior Condition
    ...
    --         Each condition must be initialized.
      ...
      method Broadcast (mutex: ptr to Mutex)
          ...
          oldIntStat = SetInterruptsTo (oldIntStat)
        endMethod
-------------------------------------------------Hoare Signal Method for assignment 4----------------------------------------------------------
      method Hoare_Signal(mutex: ptr to Mutex)
      -- Hoare semantic for Signal method
      -- the only difference between this function and the signal (mesa semantic) is that the hoare semantic hands the mutex off to the next thread by using the heldBy variable and put the current thread to the front of the wait list 
            var
                oldIntStat: int
                t: ptr to Thread
            oldIntStat = SetInterruptsTo(DISABLED)
            ...
            if t
                ...
                readyList.AddToEnd (t)
            endIf
            ---Hand the lock off to the next thread
            mutex.heldBy = t -- give held by t
            waitingThreads.AddToFront(currentThread) -- add the current thread to the end of the waiting list
...
-----------------------------  ThreadManager  ---------------------------------
  ...
  behavior ThreadManager
      ...
      method Init ()
        ...
        -- Initializes the global variables as described in assignment
          var
            i:int
          print ("Initializing Thread Manager...\n")
          threadTable = new array of Thread {MAX_NUMBER_OF_PROCESSES of new Thread} -- 10 threads allowed on the heap
          freeList = new List[Thread] -- holds the references of the threads that are free
          threadTable[i].Init("0") -- call init
          threadTable[i].Init("1")
          threadTable[i].Init("2")
          threadTable[i].Init("3")
          threadTable[i].Init("4")
          threadTable[i].Init("5")
          threadTable[i].Init("6")
          threadTable[i].Init("7")
          threadTable[i].Init("8")
          threadTable[i].Init("9") 
          for i = 0 to MAX_NUMBER_OF_PROCESSES - 1 
            threadTable[i].status = UNUSED --initial status
            freeList.AddToEnd(&threadTable[i]) -- all threads are free at the beginning
          endFor
      ...
      method Print ()
        ...
        var i, oldStatus: int
          ...
          for i = 0 to MAX_NUMBER_OF_PROCESSES-1
            ...
            printInt (i)
            print (":")
            --ThreadPrintShort (&threadTable[i]) -- PLEASE DO NOT UNCOMMENT THIS LINE IT THROWS A UNRECOVERABLE ERROR
          endFor
      ...
      method GetANewThread () returns ptr to Thread
        ...
        -- a process calls this function trying to get a new thread
        -- once it is called and the the function locks then that process 
        -- has a priority to get a new thread in the event of  a context switch
        -- and another process calls the same function
        -- the function creates a variable new thread and checks the free list using
        -- mesa semantic if the list is empty then it waits otherwise the list has a member
        -- which it returns. 
          var
            newThread: ptr to Thread
          threadManagerLock.Lock()
          newThread = null -- set the initial value to null
          while (freeList.IsEmpty()) --mesa semantic used here to check for empty list
            aThreadBecameFree.Wait(&threadManagerLock) --wait while the list is empty
          endWhile
          --pop a thread off the freelist
          newThread = freeList.Remove() --pops a thread off the list of threads
          newThread.Init("new thread") -- initialize this thread (for some reason, the tests doesn't work without this here, I think it would be harmless  to reinitialize ...maybe?... to do this here
          newThread.status = JUST_CREATED --set the thread status as just created
      ...
      method FreeThread (th: ptr to Thread)
        ...
        -- to the FREE list.
        -- it sets the status of a thread to unused and then puts it in the end of the freelist
        -- then signal the other threads to run
           threadManagerLock.Lock() --lock the critical section otherwise it may stop the action of another thread
           th.status = UNUSED --set status to unused
           freeList.AddToEnd(th) --add a thread to the end of the freelist
           aThreadBecameFree.Signal(&threadManagerLock) --signal other threads
           threadManagerLock.Unlock() 
        endMethod
...
-----------------------------  ProcessManager  ---------------------------------
  ...
  behavior ProcessManager
      ...
      method Init ()
        ...
        -- This method only initializes all the global variables
        --
        var
            i:int 
        ...
        for i = 0 to MAX_NUMBER_OF_PROCESSES -1
            processTable[i].Init() --init
            freeList.AddToEnd(&processTable[i]) -- set pointers to the arry
        endFor
        nextPid = 1 -- pid starts and one and incrementally proceeds
        processManagerLock = new Mutex
        processManagerLock.Init() --Mutex Initializer
        aProcessBecameFree = new Condition
        aProcessBecameFree.Init() -- Condition Initializer
        aProcessDied = new Condition -- not used as per instructions
        aProcessDied.Init()
        endMethod
      ...
      method GetANewProcess () returns ptr to ProcessControlBlock
        ...
        -- until one is available.
        -- this method creates a local variable to process control block 
        -- it checks if the free list has a resource available, it waits if it is empty
        -- next it gets a new process from the freelist by removing a node and setting pid and status states
        -- next it increments pid and unlocks the critical seciton
          var
            p: ptr to ProcessControlBlock
          processManagerLock.Lock() --lock the critical section
          while (freeList.IsEmpty())-- check if the free list is empty, if it isn't then wait
            aProcessBecameFree.Wait(&processManagerLock)
          endWhile
          p = freeList.Remove() -- remove a process control block from list
          --these logical steps were retrieved from instructions
          p.pid = nextPid --set current pid (this is unique)
          p.status = ACTIVE -- set the process as active as per instruction
          nextPid = nextPid + 1 --increment pid
      ...
      method FreeProcess (p: ptr to ProcessControlBlock)
        ...
        -- to the FREE list.
        -- this code basically sets the process status to free and then
        -- adds it to the end of the freelist and signals other processes
           processManagerLock.Lock() --lock the critical section
           p.status = FREE  --set status to FREE
           freeList.AddToEnd(p) --add a process to the end of the free queue
           --p.Print() --print process information and the current process list
           --print(" process now free")
           --self.Print()
           aProcessBecameFree.Signal(&processManagerLock) --signal other processes
           processManagerLock.Unlock() 
        endMethod
...
-----------------------------  FrameManager  ---------------------------------
  ...
  behavior FrameManager
      ...
      method GetNewFrames (aPageTable: ptr to AddrSpace, numFramesNeeded: int)
        -- this method takes a pointer to the address space from where the pages are stored and 
        -- the number of frames that it needs from the resource
        -- the algorithm for this method is described in the project
        -- the locks are so that no other process steals the frames from the process holding the lock
        var
            i:int
            bit_num: int
            free_frame:int
        frameManagerLock.Lock() --lock
        --wait for new frames to be available
        while(numFramesNeeded > numberFreeFrames) --mesa semantic used here for simplicity
            newFramesAvailable.Wait(&frameManagerLock)
        endWhile
        for i= 0 to numFramesNeeded -1
            --find the zero bit and set the bit to one return the bit num 
            -- this will find the first free frame address that is available for use
            bit_num = framesInUse.FindZeroAndSet()
            --find the offset of the free frame
            free_frame = PHYSICAL_ADDRESS_OF_FIRST_PAGE_FRAME +(bit_num * PAGE_SIZE)
            --set the page frame
            aPageTable.SetFrameAddr(i, free_frame)
        endFor
        --decrement the free pages so that condition variable remains accurate
      ...
      method ReturnAllFrames (aPageTable: ptr to AddrSpace)
        -- this method takes the page table and finds the number of pages where the bit is set to one
        -- using this the address of the physical frame is extracted (see for loop)
        -- once all the bits have been cleared the number of free frames are updated
        -- then the broadcast function is called to wake all processes that need a frame
        var
            i:int
            numFramesReturned:int
            frameAddr:int
            bit_num:int
        numFramesReturned = aPageTable.numberOfPages -- get the number of bits marked one
        frameManagerLock.Lock() --lock critical section
        for i = 0 to numFramesReturned -1
            frameAddr = aPageTable.ExtractFrameAddr(i) -- get the address of the bit marked 1
            bit_num = (frameAddr - PHYSICAL_ADDRESS_OF_FIRST_PAGE_FRAME) / PAGE_SIZE -- solve for the bitnum from formula above
            framesInUse.ClearBit(bit_num) -- set the bit to 0
        endFor
        numberFreeFrames = numberFreeFrames + numFramesReturned -- add all pages back
        newFramesAvailable.Broadcast(&frameManagerLock) -- wake all threads up
        frameManagerLock.Unlock()
        endMethod
...
----------------------------------------------------------------------------
----------------------------------------------------------------------------
------------- Sys call wrapper functions implentation start here--------------
----------------------------------------------------------------------------
----------------------------------------------------------------------------
-----------------------------  Handle_Sys_Exit  ---------------------------------

  function Handle_Sys_Exit (returnStatus: int)
        --exit on a user process
        --return status as integer in argument
        --returns void
        print("Syscall 'Exit' was invoked by a user thread, return status:  ")
...
-----------------------------  Handle_Sys_Shutdown  ---------------------------------
  ...
  function Handle_Sys_Shutdown ()
        --just returns a fatalerror as described in instructions
...
-----------------------------  Handle_Sys_Yield  ---------------------------------
  ...
  function Handle_Sys_Yield ()
        --wrapper for the system call yield
        --prints invocation
        print ("Syscall 'Yield' was invoked by a user thread")
        print("\n")
    endFunction
...
-----------------------------  Handle_Sys_Fork  ---------------------------------
  ...
  function Handle_Sys_Fork () returns int
      --forks on two processes 
      --returns 1000 and prints invocation
      print("Syscall 'Fork' was invoked by a user thread")
      return 1000
    endFunction
...
-----------------------------  Handle_Sys_Join  ---------------------------------
  ...
  function Handle_Sys_Join (processID: int) returns int
      --joins two processes as a wrapper function and returns int
      print("Syscall 'Join' was invoked by a user thread on process ID: ")
      printInt(processID)
      print("\n")
      return 2000
    endFunction
...
-----------------------------  Handle_Sys_Exec  ---------------------------------
  ...
  function Handle_Sys_Exec (filename: ptr to array of char) returns int
    ...
    --takes a executable filename passed as an argument and assigns it to a new address space for the process
    --sanity check on file is done on the first part of the function
    --the latter part of the function is followed from pages 27-30 in the instructions
    --FatalErrors are replaced with returns so that execution is not halted during the returns
    --Initially they were put in as a mistake because I thought they would return to normal execution after the exception
    var
        ...
        old: int
    --perform sanity check on file
    --import string from user space to kernel space
    i = currentThread.myProcess.addrSpace.GetStringFromVirtual(&strBuffer, filename asInteger, MAX_STRING_SIZE)
    if i < 0
        --FatalError("error encountered when copying string")
        return -1
    endIf      
    --check if file exists
    filePtr = fileManager.Open(&strBuffer)
    if filePtr == null
        --FatalError("error encountered when opening file")
        return -1
    endIf
    --end sanity check here
    newAddrSpace.Init() -- initialize new address space
    -- create address space for the new program and close file
    -- followed by error check
    initPC = filePtr.LoadExecutable(& newAddrSpace)
    fileManager.Close(filePtr)
    --this is needed when exec opens a non existent file name
    --added after the testing
    if initPC < 0
       -- FatalError("Error encountered when loading executable file")
       return -1
    endIf
    frameManager.ReturnAllFrames(&currentThread.myProcess.addrSpace) --free all frames
    currentThread.myProcess.addrSpace = newAddrSpace --copy the addrSpace to pcb
    --followed from the StartUserProcess function
    userStack = newAddrSpace.numberOfPages * PAGE_SIZE 
    initSystemStackTop = &currentThread.systemStack[SYSTEM_STACK_SIZE-1]
    old = SetInterruptsTo (DISABLED)
    newAddrSpace.SetToThisPageTable()
    currentThread.isUserThread = true
    BecomeUserThread(userStack, initPC, initSystemStackTop asInteger)
    return 3000
    endFunction
...
-----------------------------  Handle_Sys_Create  ---------------------------------
  ...
  function Handle_Sys_Create (filename: ptr to array of char) returns int
    --wrapper function for creating a file
    --code for moving string from user to kernel space was copied from the instructions given in assignment
    --this function takes a char* and prints the create syscall information
    var
        strBuffer: array[MAX_STRING_SIZE] of char
        i:int
    i = currentThread.myProcess.addrSpace.GetStringFromVirtual(&strBuffer, filename asInteger, MAX_STRING_SIZE)
    if i < 0
        FatalError("error encountered when creating file")
    endIf
    print("Syscall 'Create' was invoked by a user thread on file: ")
    print(&strBuffer)
    return 4000
    endFunction
...
-----------------------------  Handle_Sys_Open  ---------------------------------
  ...
  function Handle_Sys_Open (filename: ptr to array of char) returns int
    ...
    --code for moving string from user to kernel space was copied from the instructions given in assignment
    --this function takes a char* and prints the open syscall information
    var
        strBuffer: array[MAX_STRING_SIZE] of char
        i:int
    i = currentThread.myProcess.addrSpace.GetStringFromVirtual(&strBuffer, filename asInteger, MAX_STRING_SIZE)
    if i < 0
        FatalError("error encountered when opening file")
    endIf
    print("Syscall 'Open' was invoked by a user thread on file: ")
    print(&strBuffer)
    return 5000
    endFunction
...
-----------------------------  Handle_Sys_Read  ---------------------------------
  ...
  function Handle_Sys_Read (fileDesc: int, buffer: ptr to char, sizeInBytes: int) returns int
     --prints file read information, the description of file as int, buffer start and size of buffer
     print("Syscall 'Read' was invoked by a user thread on file description: ")
     printInt(fileDesc)
     print("\n")
     print("input buffer address starts at: ")
...
-----------------------------  Handle_Sys_Write  ---------------------------------
  ...
  function Handle_Sys_Write (fileDesc: int, buffer: ptr to char, sizeInBytes: int) returns int
   --prints file write information, the description of file as int, buffer start and size of buffer

     print("Syscall 'Write' was invoked by a user thread on file description: ")
     printInt(fileDesc)
     print("\n")
     print("input buffer address starts at: ")
     printHex(buffer asInteger)
...
-----------------------------  Handle_Sys_Seek  ---------------------------------
  ...
  function Handle_Sys_Seek (fileDesc: int, newCurrentPos: int) returns int
     --this function is invokes a seek on a file given in the arguments
     --arguments: file description as int and the current seek position as int
     --returns 8000
     print("Syscall 'Seek' was invoked by a user thread on file description: ")
     printInt(fileDesc)
     print("\n")
...
-----------------------------  Handle_Sys_Close  ---------------------------------
  ...
  function Handle_Sys_Close (fileDesc: int)
    -- close system call wrapper
    --takes a file description as int
    -- returns void
    --prints the invocation and file description as int
     print("Syscall 'Close' was invoked by a user thread on file description: ")
     printInt(fileDesc)
     print("\n")
    endFunction
----------------------------------------------------------------------------
----------------------------------------------------------------------------
------------- Sys call wrapper functions implentation end here--------------
----------------------------------------------------------------------------
----------------------------------------------------------------------------
-----------------------------  DiskDriver  ---------------------------------
...
-----------------------------  OpenFile  ---------------------------------
  ...
  endBehavior
---------------------------------------------------------------------------------------
-----------------------USER LEVEL PROCESS INITIALIZATION CODE--------------------------
---------------------------------------------------------------------------------------
  function InitFirstProcess()
    --Init first process just asks for thread from the thread manager object
    --initializes the thread to for a user process
    --Forks it to start the process
    var
        th: ptr to Thread 
    th = threadManager.GetANewThread()
    th.Init("UserlevelProgram")
    th.Fork(StartUserProcess, 0)
    endFunction
  
  function StartUserProcess()
    --This function gets a process control block
    --it opens the executable file and runs it on a user level
    --description comes from the instructions pg27-28
    --FatalErrors will cause the halt of the kernel
    var
    ...
    --initialize myThread and myProcess
    PCB.myThread = currentThread
    currentThread.myProcess = PCB
    --open file
    filePtr = fileManager.Open("TestProgram1")
    --sanity check on opening the file
    if filePtr == null
        FatalError("error during opening file")
    endIf
    --load file to entry point initPC
    initPC = filePtr.LoadExecutable(&PCB.addrSpace)
    if initPC < 0
        FatalError("error establishing entry point")
    endIf
    --close the file
    fileManager.Close(filePtr)
    --get the logical address of the user stack space
    InitUserStackTop = PCB.addrSpace.numberOfPages*PAGE_SIZE
    --get address for the system stack top using the address of current thread
    initSystemStackTop = &currentThread.systemStack[SYSTEM_STACK_SIZE-1]
    --run a user level thread
    --disable interrupts
    old = SetInterruptsTo(DISABLED)
    --initialize page table registers
    PCB.addrSpace.SetToThisPageTable()
    --set isUserThread to true
    currentThread.isUserThread = true
    --change mode bits for atomic jump
