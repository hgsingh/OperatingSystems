...
-----------------------------  ProducerConsumer  ---------------------------------
  ...
  var
    ...
    pcLock: Mutex = new Mutex --lock variable declared here
    emptySem:Semaphore = new Semaphore --semaphore for the empty slots in the array
    fullSem:Semaphore = new Semaphore 
  function ProducerConsumer ()

      print ("     ")
      pcLock.Init(false)
      fullSem.Init(0)
      emptySem.Init(BUFFER_SIZE)  
  ...
  function Producer (myId: int)
      ...
      for i = 1 to 5
        ...
        emptySem.Down()-- use semaphore condition
        pcLock.Lock() -- use inner mutex lock
        -- Add c to the buffer
  ...
  function Consumer (myId: int)
      ...
      while true
        -- Perform synchroniztion...
        fullSem.Down()-- use semaphore condition
        pcLock.Lock()-- use inner mutex lock
        ...
        -- Print a line showing the state
        PrintBuffer (c)
        pcLock.Unlock()--inner mutex lock
        emptySem.Up()--outer semaphore condition
        -- Perform synchronization...
...
-----------------------------  Dining Philosophers  ---------------------------------
  ...
  class ForkMonitor
    ...
    fields
      ...
      monitorLock:Mutex  -- Mutex needed for the condition
      testCond:Condition --condition variable        
    methods
      Init () 
      PickupForks (p: int)
      PutDownForks (p: int)
  ...
  behavior ForkMonitor
    ...
    method Init ()
      ...
      testCond = new Condition
      monitorLock.Init(false)
      testCond.Init()
      status = new array of int{HUNGRY, HUNGRY, HUNGRY, HUNGRY, HUNGRY}
      endMethod
    ...
    method PutDownForks (p: int)
      -- This method is called when the philosopher 'p' is done eating.
        p = p % 5 
        monitorLock.Lock()
        if status[(p+1) % 5] == HUNGRY && status[p] != EATING && status[(p + 2) % 5] != EATING
            status[(p+1) % 5] = EATING
            testCond.Wait(&monitorLock)
        endIf
        if status[(p - 1)% 5] == HUNGRY && status[(p -2) % 5] != EATING && status[p ] != EATING
            status[(p - 1) % 5] = EATING
            testCond.Wait(&monitorLock)
        endIf
        monitorLock.Unlock()
