  ...
  behavior Mutex
    ...
    --         on the mutex.
      ...
      ----------  Mutex . Init  ----------
        ...
        method Init (newState:bool)
            if(newState == true)
                FatalError ("Cannot create a state with a held flag")
            endIf
            state = newState
            heldBy = null
            waitingThreads = new List [Thread]
      ...
      ----------  Mutex . IsHeldByCurrentThread  ----------
        ...
        method IsHeldByCurrentThread () returns bool
            if heldBy == currentThread 
                print("Lock already being held by a current thread")
                return true 
            else
                return false
            endIf
      ...
      ----------  Mutex . Lock  ----------
        ...
        method Lock ()
          --simlar to semaphore.down
            var
                oldIntStat: int
            oldIntStat = SetInterruptsTo(DISABLED)
            if heldBy == currentThread
                 FatalError ("Cannot lock thread") 
            endIf
            while state == true 
                waitingThreads.AddToEnd(currentThread)
                currentThread.Sleep()
            endWhile
            state = true --lock aquired
      ...
      ----------  Mutex . Unlock  ----------
        ...
        method Unlock ()
            --similar to semaphore.up
            var
                oldIntStat: int
                t: ptr to Thread
            oldIntStat = SetInterruptsTo(DISABLED)
            if heldBy != currentThread
                FatalError("Cannot unlock")
            endIf  
            --print("WE GOT IN TO THE UNLOCKING MESSHOLE \n")
            t = waitingThreads.Remove()
            if t != null        
                t.status = READY
                readyList.AddToEnd(t) 
